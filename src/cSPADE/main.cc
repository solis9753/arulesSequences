...
int main(int argc, char **argv)
{
   ...
   parse_args(argc, argv);
    ...
    nameFile = "sparse";
    SparseFile.open(nameFile, ios::out | ios::binary);

    if(SparseFile.is_open()) {
        SparseFile.write((const char *)&(DBASE_NUM_TRANS), sizeof(int));
        SparseFile.write((const char *)&(numFreqPatterns), sizeof(int));
        SparseFile.write((const char *)&(numNonZeroElements), sizeof(int));
   ...
   newSeq();
   seconds(t2);
    SparseFile.open(nameFile, ios::out | ios::binary | ios::in);
